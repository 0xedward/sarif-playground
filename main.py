...
CONFIG: Dict[str, Union[bytes, str]] = {
...
def pad(value, bs: int=BLOCKSIZE):
    pv = bs - (len(value) % bs)
...
def listservices() -> str:
    ...
    try:
        # reveal_taint(query_build('SELECT * from public_stuff{}'.format(where)))
        reveal_taint(query_build(f'SELECT * from public_stuff{where}'))
        cursor.execute(query_build(f'SELECT * from public_stuff{where}'))
...
if __name__ == "__main__":
    ...
    try:
        if args.database_type not in ['sqlite', 'oracle']:
            connection_params: Dict[str, Any] = {a.replace('database_', ''): getattr(args, a) for a in dir(args) if a in ['database_user', 'database_host', 'database_password', 'database_port']}
            connection = dbmodule.connect(**connection_params)
            # reveal_type(connection)
            ...
            connection_params: Dict[str, Any] = {a.replace('database_', ''): getattr(args, a) for a in dir(args) if a in ['database_user', 'database_password']}
